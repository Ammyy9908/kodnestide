...
function App() {
  ...
  const [language, setLanguage] = useState(93);
  const [output, setOutput] = useState("");
  const [status, setStatus] = useState("");
  useEffect(() => {
    document.title = "KodIDE";
  }, []);

  let d = data["languages"].filter((l) => l.id === +language);
  ...
  return (
    <div className="overflow-hidden">
      <Header setLanguage={setLanguage} setTheme={setTheme} theme={theme} />
      <main
        className={`w-full grid grid-cols-4 overflow-hidden ${
          theme === "vs-dark"
            ? "bg-[#00030B] text-white"
            : "bg-white text-[#00030B]"
        }`}
      >
        ...
        <div className=" h-full grid grid-row-span-8 gap-2">
          <div className={`   px-2 py-3  overflow-y-scroll row-span-3`}>
            <pre>
              <code>{output}</code>
            </pre>
          </div>
          ...
          >
            <h2>Custom Input</h2>
            <textarea
              className={`w-full  h-full resize-none outline-none border-2 border-gray-300 ${
                theme === "vs-dark"
                  ? "bg-[#00030B] text-white"
                  : "bg-white text-[#00030B]"
              }`}
      ...
      <div
        className={`fixed bottom-10 left-10 ${
          ...
          theme === "vs-dark" ? "bg-black" : "bg-gray-200"
        } shadow-2xl rounded-full flex items-center gap-2 transition-all`}
      >
        <button
          className={`bg-purple-600 px-4  text-white w-12 h-12  rounded-full shadow-2x`}
          onClick={handleExecution}
        >
          {loader ? <Spinner /> : <FaPlay />}
        </button>
        {status && (
          <span
            className={`font-bold ${
